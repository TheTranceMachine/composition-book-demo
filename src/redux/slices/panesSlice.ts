...
export const panesSlice = createSlice({
  ...
  reducers: {
    ...
    removeGroupedPane: (state, action: PayloadAction<{ paneId: string; groupPaneId: string }>) => {
      return state.map((pane) =>
        pane.id === action.payload.paneId
          ? { ...pane, group: pane.group.filter((group) => group.id !== action.payload.groupPaneId) }
          : pane
      );
    },
      ...
      return state.map((pane) =>
        pane.id === action.payload.paneId
          ? {
              ...
              group: pane.group.map((group) =>
                group.id === action.payload.groupPaneId
                  ? { ...group, tabs: [...group.tabs, action.payload.tab] }
                  : group
              ),
            }
          : pane
      ...
      return state.map((pane) =>
        pane.id === action.payload.paneId
          ? {
              ...
              group: pane.group.map((group) =>
                group.id === action.payload.groupPaneId
                  ? { ...group, tabs: group.tabs.filter((tab) => tab.id !== action.payload.tabId) }
                  : group
              ),
            }
          : pane
    ...
    updateGroupTab: (
      state,
      action: PayloadAction<{ paneId: string; groupPaneId: string; tabId: string; name: string }>
    ) => {
      return state.map((pane) =>
        pane.id === action.payload.paneId
          ? {
    ...
    updateTabContent: (
      state,
      action: PayloadAction<{ paneId: string; tabId: string; content: string | undefined }>
    ) => {
      return state.map((pane) =>
        pane.id === action.payload.paneId
          ? {
    ...
    sortTabs: (state, action: PayloadAction<{ paneId: string; tabs: TabTypes[] }>) => {
      return state.map((pane) => (pane.id === action.payload.paneId ? { ...pane, tabs: action.payload.tabs } : pane));
    },
      ...
      return state.map((pane) =>
        pane.id === action.payload.paneId
          ? {
              ...pane,
              group: pane.group.map((group) =>
                group.id === action.payload.groupPaneId ? { ...group, tabs: action.payload.tabs } : group
              ),
            }
          : pane
      );
      ...
      return state.map((pane) =>
        pane.id === action.payload.paneId
          ? {
              ...pane,
              group: pane.group.map((group) =>
                group.id === action.payload.groupPaneId ? { ...group, size: action.payload.size } : group
              ),
            }
          : pane
      );
